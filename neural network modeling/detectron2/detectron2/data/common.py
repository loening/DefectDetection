...
class MapDataset(data.Dataset):
    ...
    def __init__(self, dataset, map_func, cfg):
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            if cfg.INPUT.Mosaic and np.random.uniform(0, 1) < cfg.INPUT.MosaicPresent:
            # if cfg.INPUT.Mosaic:
                mosaic = T.LoadMosaic(cur_idx, cfg, self._dataset)
                data, is_aug = mosaic.get_transform(data)
                if is_aug:
                    self._dataset[cur_idx]["height"] = cfg.INPUT.MosaicSize
                    self._dataset[cur_idx]["width"] = cfg.INPUT.MosaicSize
                data = self._map_func(data)
            else:
                data = self._map_func(data)
            if data is not None:
