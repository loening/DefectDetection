...
def build_augmentation(cfg, is_train):
    ...
    if is_train:
        ...
        if cfg.INPUT.Brightness:
            augmentation.append(T.RandomBrightness(cfg.INPUT.Brightness[0], cfg.INPUT.Brightness[1]))
        if cfg.INPUT.Contrast:
            augmentation.append(T.RandomContrast(cfg.INPUT.Contrast[0], cfg.INPUT.Contrast[1]))
        if cfg.INPUT.HorFlip:
            augmentation.append(T.RandomFlip(prob=cfg.INPUT.HorFlip, horizontal=True, vertical=False))
        if cfg.INPUT.VerFlip:
            augmentation.append(T.RandomFlip(prob=cfg.INPUT.VerFlip, horizontal=False, vertical=True))
        if cfg.INPUT.Rotation:
            augmentation.append(T.RandomRotation(cfg.INPUT.Rotation, expand=False, sample_style="choice"))
        if cfg.INPUT.Gauss:
            augmentation.append(T.RandomGauss(cfg.INPUT.Gauss[0], cfg.INPUT.Gauss[1]))
