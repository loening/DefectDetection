...
def gen_crop_transform_with_instance(crop_size, image_size, instances, crop_box=True):
    ...
    if len(instances):
        ...
        bbox = np.clip(bbox, 0, max(bbox))
        crop_size = np.asarray(crop_size, dtype=np.int32)
        # crop_size = np.asarray(((bbox[3] - bbox[1]) * 1.5, (bbox[2] - bbox[0]) * 1.5), dtype=np.int32)
        # print(crop_size)
        ...
        # crop_size = np.asarray((800, 800), np.int32)
