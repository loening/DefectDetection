...
class Blender(object):
    ...
    def __call__(self, bases, proposals, gt_instances):
        if gt_instances is not None:
            ...
            # torch.save(bases[0], r"/media/ps/data/train/LQ/LQ/blendmask/workspace/bases.data")
            # torch.save(attns, r"/media/ps/data/train/LQ/LQ/blendmask/workspace/top_feat.data")
            # with open("/media/ps/data/train/LQ/LQ/bdmask4/workspace/mask_data/pdata/top_feat.data", 'wb') as f_top_feat:
            #     f_top_feat.write(attns.cpu().data.numpy().tobytes())
            ...
            # with open("/media/ps/data/train/LQ/LQ/bdmask4/workspace/mask_data/pdata/pred_mask_logits.data", 'wb') as f_pred_mask_logits:
            #     f_pred_mask_logits.write(pred_mask_logits.cpu().data.numpy().tobytes())
            
            # torch.save(pred_mask_logits, r"/media/ps/data/train/LQ/LQ/blendmask/workspace/pred_mask_logits.data")
            start_ind = 0
            for box in proposals:
    ...
    def merge_bases(self, rois, coeffs, location_to_inds=None):
        ...
        # path = '/media/ps/1616B1B516B19669/LB/train/HG2_project/blendmask/bottom48/out'
        # if not os.path.exists(path):
        #     os.makedirs(path)
        # f = FeatureVisualization(coeffs,path)
        # f.get_feature()
