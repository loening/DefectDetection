...
class FCOSOutputs(nn.Module):
    def __init__(self, cfg):
        ...
        # additional parameters of targets for locations
        self.train_one_class = cfg.MODEL.FCOS.ONE_CLASS
        self.use_compression = cfg.MODEL.FCOS.USE_COMPRESSION
        self.cmprs_width = cfg.MODEL.FCOS.CMPRS_WIDTH
        self.cmprs_aspect_ratio = cfg.MODEL.FCOS.CMPRS_ASPECT_RATIO
    ...
    def compute_targets_for_locations(self, locations, targets, size_ranges, num_loc_list):
        '''
        locations: torch.Size([87296, 2])
        targets: [Instances(num_instances=1, image_height=2048, image_width=2048, fields=[gt_boxes: Boxes(tensor([[1156.,  269., 1199.,  315.]], device='cuda:0')), gt_classes: tensor([0], device='cuda:0'), gt_masks: PolygonMasks(num_instances=1)]), Instances(num_instances=1, image_height=2048, image_width=2048, fields=[gt_boxes: Boxes(tensor([[ 941.,  382., 1026.,  478.]], device='cuda:0')), gt_classes: tensor([0], device='cuda:0'), gt_masks: PolygonMasks(num_instances=1)])]
        size_ranges: tensor([[-1.0000e+00,  6.4000e+01],
                ...,
                [ 5.1200e+02,  1.0000e+08]], device='cuda:0')
        num_loc_list: [65536, 16384, 4096, 1024, 256]
        '''
        labels = []
        reg_targets = []
        target_inds = []
        ...
        for im_i in range(len(targets)):
            ...
            # TODO: change the label to single category
            if self.train_one_class:
                labels_per_im = torch.zeros_like(labels_per_im)
            ...
            # TODO: Optimize linear defect detection, Change the detected feature layer
            # print("------------------", bboxes.shape)
            if self.use_compression:
                a = torch.stack([bboxes[:, 2][None] - bboxes[:, 0][None], bboxes[:, 3][None] - bboxes[:, 1][None]], dim=2)
                w = a.min(dim=2)[0]
                h = a.max(dim=2)[0]
                r = w / h
                one = torch.ones_like(r)
                # 宽高比阈值
                r = torch.where(r >= self.cmprs_aspect_ratio, one, r)
                # 宽度阈值
                r = torch.where(w > self.cmprs_width, one, r)
                alpha = torch.log(r + 1) / torch.log(one * 2)
                max_reg_targets_per_im = alpha * reg_targets_per_im.max(dim=2)[0]
            else:
    ...
    def fcos_losses(self, instances):
        ...
        # zero = torch.zeros_like(instances.reg_targets)
        # instances.reg_targets = torch.where(instances.reg_targets >= 0, instances.reg_targets, zero)
    ...
    def predict_proposals(
        ...
        boxlists1 = self.select_over_all_levels(boxlists)
    ...
    def forward_for_single_feature_map(
        ...
        logits_pred_temp = logits_pred.clone()
        ...
        results = []
        for i in range(N):
            per_box_cls = logits_pred[i]   # torch.Size([65536, 25])
            per_candidate_inds = candidate_inds[i] # torch.Size([65536, 25])
            per_box_cls = per_box_cls[per_candidate_inds] # 得到类别的得分
            # per_box_cls_temp = logits_pred_temp[i][per_candidate_inds]

            per_candidate_nonzeros = per_candidate_inds.nonzero(as_tuple=False)
            per_box_loc = per_candidate_nonzeros[:, 0]  # 得到哪些符合条件的候选框的index
            per_class = per_candidate_nonzeros[:, 1]    # 得到缺陷的类别
            # per_box_ctrness = ctrness_pred[i][per_box_loc]
            ...
            if per_candidate_inds.sum().item() > per_pre_nms_top_n.item():
                ...
                # per_box_cls_temp = per_box_cls_temp[top_k_indices]
            ...
            # boxlist.ctr_scores = per_box_ctrness
            # boxlist.cls_scores = per_box_cls_temp
